...
- **Smooth crossfade navigation** — Framer Motion's `AnimatePresence` fades between the category, color, and zoom views.
- **Graceful empty/fallback states** — a category with no photographed colors yet shows an empty-state message instead of a blank grid; a missing image falls back to a solid swatch. Dropping a real photo into `public/images/` requires no component changes.
...
- **[Framer Motion](https://www.framer.com/motion/)** — crossfade view transitions and tap feedback
...
src/
  ...
  App.jsx               # navigation state + crossfade view switching
```
