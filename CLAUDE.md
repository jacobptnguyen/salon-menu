...
- **If using `sips` (no Pillow available) to fix orientation, do the rotate and the resize/format conversion in one pipeline, then verify the *final* committed file — not an intermediate temp file.** `sips -r <deg>` on a HEIC-derived image can leave a residual orientation tag behind even after physically rotating the pixels; a *separate*, later `sips -s format jpeg -Z <px>` pass on that intermediate file can then silently re-apply an extra 90° turn, so the temp file you eyeballed right after rotating looks correct but the final exported JPEG is off by 90° again (this bit us on the Double Rhythm "Chocolaté Café" / "Navy Blue" photos, 2026-08-26). Always re-open and visually check the exact file that lands in `/public/images/collections/` after every `sips` step that touches it, not just the intermediate one.
